...
public class BANKAPPGUI extends BASEFRAME implements ActionListener{
    ...
    protected void addGuiComponents() {
        ...
        settingsButton.setBounds(15, 450, getWidth() - 50, 50);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        BANKAPPINTERACTION bankingAppInteraction = new BANKAPPINTERACTION(this, user);

       
        bankingAppInteraction.setTitle(buttonPressed);

      
        if(buttonPressed.equalsIgnoreCase("Deposit") || buttonPressed.equalsIgnoreCase("Withdraw")
                    || buttonPressed.equalsIgnoreCase("Transfer")){
          
            bankingAppInteraction.addCurrentBalanceAndAmount();

           
            bankingAppInteraction.addActionButton(buttonPressed);

            
            if(buttonPressed.equalsIgnoreCase("Transfer")){
                bankingAppInteraction.addUserField();
            }

        }else if(buttonPressed.equalsIgnoreCase("Past Transaction")){
            bankingAppInteraction.addPastTransactionComponents();
        }

       
        bankingAppInteraction.setVisible(true);
    }
}
